...
public class IAgilityArmor extends ItemArmor implements ISpecialArmor {

	
	public IAgilityArmor(ArmorMaterial armorMaterial, int renderIndex, int armorType) {
		...
		if(armorType == 0) this.setMaxDamage(Config.AgArmorHelmetDurability);
			if(armorType == 1) this.setMaxDamage(Config.AgArmorChestPlateDurability);
				if(armorType == 2) this.setMaxDamage(Config.AgArmorLeggingsDurability);
					if(armorType == 3) this.setMaxDamage(Config.AgArmorBootsDurability);
	...
	public String getArmorTexture(ItemStack stack, Entity entity, int slot, String type)
	{
	    ...
	    return "thunder:textures/models/armor/" + "AgilityArmor" + suffix + ".png";
	}
	...
	public void onArmorTick(World world, EntityPlayer player, ItemStack itemStack){
		...
		if(this.armorType == 0){
			...
			if(player.inventory.armorItemInSlot(3) != null && player.inventory.armorItemInSlot(3).getItem() == ArmorList.AgilityArmorHelmet &&
					player.inventory.armorItemInSlot(2) != null && player.inventory.armorItemInSlot(2).getItem() == ArmorList.AgilityArmorChestPlate &&
						player.inventory.armorItemInSlot(1) != null && player.inventory.armorItemInSlot(1).getItem() == ArmorList.AgilityArmorLeggings &&
							player.inventory.armorItemInSlot(0) != null && player.inventory.armorItemInSlot(0).getItem() == ArmorList.AgilityArmorBoots){

				this.effectPlayer(player, Potion.moveSpeed, Config.AgPotionPowerSpeed);
				this.effectPlayer(player, Potion.jump, Config.AgPotionPowerJump);
				if(Config.AgFallDamage == true){
	...
	public void damageArmor(EntityLivingBase entity, ItemStack stack, DamageSource source, int damage, int slot) {
		...
		if(Config.AgConstDamage == false){
		stack.damageItem(damage * durabilityPerDamage() , entity);
		}else
		{
			if(Config.AgConstDamage == true)
	 ...
	 private double getDamageAbsorptionRatio()
	  {
	    return Config.AgDamageAbsorptionRatio;
	  }
	 
	 private int durabilityPerDamage(){
		 
		 return Config.AgDurabPerDamage;
	 }
