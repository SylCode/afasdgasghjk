...
public class IEnergyUpdaterAutoFood extends IEnergyUpdater implements IEnergyAdd {
	
	IEnergyUpdaterAutoFood(){
		super();
			...
			if(matrix != null && (matrix.getItem().equals(UpdatersList.EnergyMatrixUltra)) && hasFullSet(player)){
				NBTTagCompound nbtData = getOrCreateNbtData(matrix);
				 ...
				 for(int a = 0; a < inv.getSizeInventory(); a++){
					...
					if(stack != null && (stack.getItem().equals(UpdatersList.EnergyUpdaterAutoFood))){
						counter += 1;
					}
				 }
				
				 if(nbtData.getBoolean("active") && ElectricItem.manager.canUse(matrix, 40000.0D) && counter == 1){
					 if(player.getFoodStats().getFoodLevel() < 20)
					 {
						 int level = 20 - player.getFoodStats().getFoodLevel();
						 player.getFoodStats().addStats(level, 2.4F);
						 ElectricItem.manager.discharge(matrix, 40000.0D, 4, true, false, false);
					 }
	...
	protected String getUpdater(){	
		return StatCollector.translateToLocal("tag.updaters.EnergyUpdaterAutoFood.getupdater");
	}
	
	@Override
	protected String getType(){
		return StatCollector.translateToLocal("tag.updaters.EnergyUpdaterAutoFood.gettype");
	}
	
	@Override
	protected String getMatrix(){
		return StatCollector.translateToLocal("tag.updaters.EnergyUpdaterAutoFood.getmatrix");
	}
	
	@Override
	protected String getEffect(){
		return StatCollector.translateToLocal("tag.updaters.EnergyUpdaterAutoFood.geteffect");
	}		
