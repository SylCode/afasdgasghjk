...
public class IEnergyUpdaterAddJumpOnHit3 extends IEnergyUpdater implements IEnergyAdd {
	
	IEnergyUpdaterAddJumpOnHit3(){
		super();
		...
		if (player != null) {
			...
			if(matrix != null && (matrix.getItem().equals(UpdatersList.EnergyMatrixUltra)) && hasFullSet(player)){
				NBTTagCompound nbtData = getOrCreateNbtData(matrix);
				 ...
				 effectPlayer(player, Potion.jump, 7);
	...
	protected String getUpdater(){	
		return StatCollector.translateToLocal("tag.updaters.EnergyUpdaterJumpOnHit3.getupdater");
	}
	
	@Override
	protected String getType(){
		return StatCollector.translateToLocal("tag.updaters.EnergyUpdaterJumpOnHit3.gettype");
	}
	
	@Override
	protected String getMatrix(){
		return StatCollector.translateToLocal("tag.updaters.EnergyUpdaterJumpOnHit3.getmatrix");
	}
	
	@Override
	protected String getEffect(){
		return StatCollector.translateToLocal("tag.updaters.EnergyUpdaterJumpOnHit3.geteffect");
	}		
