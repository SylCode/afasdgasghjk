...
public class IEnergyUpdaterFallDamage3 extends IEnergyUpdater implements IEnergyAdd {
	
	IEnergyUpdaterFallDamage3(){
		super();
			...
			if(matrix != null && (matrix.getItem().equals(UpdatersList.EnergyMatrixUltra)) && hasFullSet(player)){
				NBTTagCompound nbtData = getOrCreateNbtData(matrix);
				 ...
				 for(int a = 0; a < inv.getSizeInventory(); a++){
					...
					if(stack != null && (stack.getItem().equals(UpdatersList.EnergyUpdaterFallDamage3))){
						counter += 1;
					}
				 }
				
				 if(nbtData.getBoolean("active") && ElectricItem.manager.canUse(matrix, 5000.0D) && counter == 1){
					
						 
						 ElectricItem.manager.discharge(matrix, 5000.0D, 4, true, false, false);
						 return true;
	...
	protected String getUpdater(){	
		return StatCollector.translateToLocal("tag.updaters.EnergyUpdaterFallDamage3.getupdater");
	}
	
	@Override
	protected String getType(){
		return StatCollector.translateToLocal("tag.updaters.EnergyUpdaterFallDamage3.gettype");
	}
	
	@Override
	protected String getMatrix(){
		return StatCollector.translateToLocal("tag.updaters.EnergyUpdaterFallDamage3.getmatrix");
	}
	
	@Override
	protected String getEffect(){
		return StatCollector.translateToLocal("tag.updaters.EnergyUpdaterFallDamage3.geteffect");
	}		
