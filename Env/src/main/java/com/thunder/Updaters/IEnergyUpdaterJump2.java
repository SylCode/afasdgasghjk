...
public class IEnergyUpdaterJump2 extends IEnergyUpdater implements IEnergyAdd {
		...
		if (entity != null && entity instanceof EntityPlayer) {
			...
			if(matrix != null && (matrix.getItem().equals(UpdatersList.EnergyMatrixStandart) || matrix.getItem().equals(UpdatersList.EnergyMatrixUltra)) && hasFullSet(player)){
				NBTTagCompound nbtData = getOrCreateNbtData(matrix);
				 ...
				 if(nbtData.getBoolean("active") && ElectricItem.manager.canUse(matrix, 60.0D) && counter == 1){
			 
					 	 effectPlayer(player, Potion.jump, 3);
						 ElectricItem.manager.discharge(matrix, 60.0D, 4, true, false, false);
	...
	protected String getUpdater(){	
		return StatCollector.translateToLocal("tag.updaters.EnergyUpdaterJump2.getupdater");
	}
	
	@Override
	protected String getType(){
		return StatCollector.translateToLocal("tag.updaters.EnergyUpdaterJump2.gettype");
	}
	
	@Override
	protected String getMatrix(){
		return StatCollector.translateToLocal("tag.updaters.EnergyUpdaterJump2.getmatrix");
	}
	
	@Override
	protected String getEffect(){
		return StatCollector.translateToLocal("tag.updaters.EnergyUpdaterJump2.geteffect");
	}		
