...
public class IEnergyUpdaterRestoreHeartOnHit3 extends IEnergyUpdater implements IEnergyAdd {
	
	IEnergyUpdaterRestoreHeartOnHit3(){
		super();
		...
		if (player != null) {
			...
			if(matrix != null && (matrix.getItem().equals(UpdatersList.EnergyMatrixUltra)) && hasFullSet(player)){
				NBTTagCompound nbtData = getOrCreateNbtData(matrix);
				 ...
				 if(nbtData.getBoolean("active") && ElectricItem.manager.canUse(matrix, 200000.0D) && counter == 1){
				 Random r = new Random();
				 int random = r.nextInt(99);
				 if(random <= 30 && random >= 10){
				 float h = player.getHealth() + 8.0F;
				 player.setHealth(h);
				 ElectricItem.manager.discharge(matrix, 200000.0D, 4, true, false, false);
				 return damage;
				 }
				 }
			}
	...
	protected String getUpdater(){	
		return StatCollector.translateToLocal("tag.updaters.EnergyUpdaterRestoreHeartOnHit3.getupdater");
	}
	
	@Override
	protected String getType(){
		return StatCollector.translateToLocal("tag.updaters.EnergyUpdaterRestoreHeartOnHit3.gettype");
	}
	
	@Override
	protected String getMatrix(){
		return StatCollector.translateToLocal("tag.updaters.EnergyUpdaterRestoreHeartOnHit3.getmatrix");
	}
	
	@Override
	protected String getEffect(){
		return StatCollector.translateToLocal("tag.updaters.EnergyUpdaterRestoreHeartOnHit3.geteffect");
	}		
