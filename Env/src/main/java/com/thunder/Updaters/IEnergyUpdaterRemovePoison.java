...
public class IEnergyUpdaterRemovePoison extends IEnergyUpdater implements IEnergyAdd {
	
	IEnergyUpdaterRemovePoison(){
		super();
			...
			if(matrix != null && (matrix.getItem().equals(UpdatersList.EnergyMatrixUltra)) && hasFullSet(player)){
				NBTTagCompound nbtData = getOrCreateNbtData(matrix);
				 ...
				 for(int a = 0; a < inv.getSizeInventory(); a++){
					...
					if(stack != null && (stack.getItem().equals(UpdatersList.EnergyUpdaterRemovePoison))){
						counter += 1;
					}
				 }
	...
	protected String getUpdater(){	
		return StatCollector.translateToLocal("tag.updaters.EnergyUpdaterRemovePoison.getupdater");
	}
	
	@Override
	protected String getType(){
		return StatCollector.translateToLocal("tag.updaters.EnergyUpdaterRemovePoison.gettype");
	}
	
	@Override
	protected String getMatrix(){
		return StatCollector.translateToLocal("tag.updaters.EnergyUpdaterRemovePoison.getmatrix");
	}
	
	@Override
	protected String getEffect(){
		return StatCollector.translateToLocal("tag.updaters.EnergyUpdaterRemovePoison.geteffect");
	}		
