...
public class IEnergyUpdaterAntiRadiation extends IEnergyUpdater implements IEnergyAdd {
	
	IEnergyUpdaterAntiRadiation(){
		super();
			...
			if(matrix != null && (matrix.getItem().equals(UpdatersList.EnergyMatrixStandart) || matrix.getItem().equals(UpdatersList.EnergyMatrixUltra)) && hasFullSet(player)){
				NBTTagCompound nbtData = getOrCreateNbtData(matrix);
				 ...
				 for(int a = 0; a < inv.getSizeInventory(); a++){
					...
					if(stack != null && (stack.getItem().equals(UpdatersList.EnergyUpdaterAntiRadiation))){
						counter += 1;
					}
				 }
				
				 if(nbtData.getBoolean("active") && ElectricItem.manager.canUse(matrix, 500.0D) && counter == 1){
			 
					 	 if(player.isPotionActive(IC2Potion.radiation.id)){					 		 
					 	 player.removePotionEffect(IC2Potion.radiation.id);
						 ElectricItem.manager.discharge(matrix, 500.0D, 4, true, false, false);
					 	 }
	...
	protected String getUpdater(){	
		return StatCollector.translateToLocal("tag.updaters.EnergyUpdaterAntiRadiation.getupdater");
	}
	
	@Override
	protected String getType(){
		return StatCollector.translateToLocal("tag.updaters.EnergyUpdaterAntiRadiation.gettype");
	}
	
	@Override
	protected String getMatrix(){
		return StatCollector.translateToLocal("tag.updaters.EnergyUpdaterAntiRadiation.getmatrix");
	}
	
	@Override
	protected String getEffect(){
		return StatCollector.translateToLocal("tag.updaters.EnergyUpdaterAntiRadiation.geteffect");
	}		
