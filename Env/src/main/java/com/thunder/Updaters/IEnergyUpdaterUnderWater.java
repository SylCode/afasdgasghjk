...
public class IEnergyUpdaterUnderWater extends IEnergyUpdater implements IEnergyAdd {
	
	IEnergyUpdaterUnderWater(){
		super();
			...
			if(matrix != null && (matrix.getItem().equals(UpdatersList.EnergyMatrixStandart) || matrix.getItem().equals(UpdatersList.EnergyMatrixUltra)) && hasFullSet(player)){
				NBTTagCompound nbtData = getOrCreateNbtData(matrix);
				 ...
				 for(int a = 0; a < inv.getSizeInventory(); a++){
					...
					if(stack != null && (stack.getItem().equals(UpdatersList.EnergyUpdaterUnderWater))){
						counter += 1;
					}
				 }
	...
	protected String getUpdater(){	
		return StatCollector.translateToLocal("tag.updaters.EnergyUpdaterUnderWater.getupdater");
	}
	
	@Override
	protected String getType(){
		return StatCollector.translateToLocal("tag.updaters.EnergyUpdaterUnderWater.gettype");
	}
	
	@Override
	protected String getMatrix(){
		return StatCollector.translateToLocal("tag.updaters.EnergyUpdaterUnderWater.getmatrix");
	}
	
	@Override
	protected String getEffect(){
		return StatCollector.translateToLocal("tag.updaters.EnergyUpdaterUnderWater.geteffect");
	}		
