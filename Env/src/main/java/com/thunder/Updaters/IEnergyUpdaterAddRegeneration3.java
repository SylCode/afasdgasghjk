...
public class IEnergyUpdaterAddRegeneration3 extends IEnergyUpdater implements IEnergyAdd {
	
	IEnergyUpdaterAddRegeneration3(){
		super();
		...
		if (entity != null && entity instanceof EntityPlayer) {
			...
			if(matrix != null && (matrix.getItem().equals(UpdatersList.EnergyMatrixUltra)) && hasFullSet(player)){
				NBTTagCompound nbtData = getOrCreateNbtData(matrix);
				 ...
				 if(nbtData.getBoolean("active") && ElectricItem.manager.canUse(matrix, 100.0D) && counter == 1){
			 
					 	 effectPlayer(player, Potion.regeneration, 1);
						 ElectricItem.manager.discharge(matrix, 100.0D, 4, true, false, false);
	...
	protected String getUpdater(){	
		return StatCollector.translateToLocal("tag.updaters.EnergyUpdaterAddRegeneration3.getupdater");
	}
	
	@Override
	protected String getType(){
		return StatCollector.translateToLocal("tag.updaters.EnergyUpdaterAddRegeneration3.gettype");
	}
	
	@Override
	protected String getMatrix(){
		return StatCollector.translateToLocal("tag.updaters.EnergyUpdaterAddRegeneration3.getmatrix");
	}
	
	@Override
	protected String getEffect(){
		return StatCollector.translateToLocal("tag.updaters.EnergyUpdaterAddRegeneration3.geteffect");
	}		
