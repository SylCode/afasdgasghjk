...
public class IEnergyUpdaterAddExtraDamage3 extends IEnergyUpdater implements IEnergyAdd {
	
	IEnergyUpdaterAddExtraDamage3(){
		super();
		...
		if (player != null) {
			...
			if(matrix != null && (matrix.getItem().equals(UpdatersList.EnergyMatrixUltra)) && hasFullSet(player)){
				NBTTagCompound nbtData = getOrCreateNbtData(matrix);
				 ...
				 if(nbtData.getBoolean("active") && ElectricItem.manager.canUse(matrix, 500000.0D) && counter == 1){
					 
				 Random r = new Random();
				 int random = r.nextInt(99);
				 if(random <= 30 && random >= 10){
				 ElectricItem.manager.discharge(matrix, 500000.0D, 4, true, false, false);
	...
	protected String getEffect(){
		return "Chance 20% to deal double damage with autoattack(500000.0 EU)";
	}		
